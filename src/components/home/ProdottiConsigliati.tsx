...
const prodotti = [
  ...
  },
  {
    ...
    icona: <Hammer className="text-oro" size={22} />,
    colore: "border-oro",
  },
  {
  ...
  },
  {
    ...
    icona: <Wrench className="text-noce" size={22} />,
    colore: "border-noce",
  },
  {
    ...
    icona: (
      <svg width="22" height="22" fill="none" viewBox="0 0 24 24" className="text-scuroMetallo">
        <rect x="4" y="10" width="16" height="7" rx="2" fill="currentColor" />
        <rect x="6" y="5" width="12" height="5" rx="2" fill="#ded9d3" />
      </svg>
    ),
    colore: "border-scuroMetallo",
  },
];

export default function ProdottiConsigliati() {
  return (
    <section id="prodotti" className="bg-biancoFerramenta w-full py-14 px-5 border-b border-cemento">
      <div className="max-w-6xl mx-auto flex flex-col items-start">
        <h2 className="font-playfair text-2xl md:text-3xl font-bold text-verdeFerramenta mb-3 pl-1">
        ...
        <div className="grid md:grid-cols-5 sm:grid-cols-2 grid-cols-1 gap-6 w-full">
          {prodotti.map((p) => (
            <div
              key={p.nome}
              className={`relative rounded-xl bg-gradient-to-br from-cemento via-white to-biancoFerramenta border-2 ${p.colore} shadow-rustic-card group transition flex flex-col overflow-hidden`}
            >
              <img
                src={p.img}
                alt={p.nome}
              ...
              <div className="px-4 py-3 flex flex-col flex-1 h-full">
                <div className="flex items-center gap-2 font-bold text-lg mb-1 font-sans text-noce drop-shadow">
                  {p.icona}
                  {p.nome}
                </div>
                <div className="text-[#786d53] text-sm font-light flex-1 mb-3">{p.descr}</div>
                <button className="flex items-center justify-center bg-verdeFerramenta text-white font-semibold px-2 py-1 rounded transition hover:bg-noce gap-1 shadow mt-auto text-sm">
        ...
        <div className="flex justify-end w-full mt-9">
          <a
            href="/prodotti"
            className="inline-flex items-center gap-2 bg-white border-2 border-oro text-verdeFerramenta text-base font-semibold px-7 py-2 rounded-md shadow hover:bg-oro hover:text-white transition"
